...
public class LogBase {
	...
	/// <summary>
	/// Constant string used for property designation on a logger for caller information
	/// </summary>
	public const string CALLER = "Caller";

	/// <summary>
	/// Constant string used for property designation on a logger for extra data properties
	/// </summary>
	public const string DATA = "Data";

	/// <summary>
	/// Constant string for the initialization message of a log
	/// </summary>
	public const string INITIALIZED = "Log Initialized";
	...
	/// <summary>
	/// Holds a value used so that the <see cref="Initialize(InitializeInfo, string, string)"/> method only executes one time to avoid log
	...
	public void Error(Exception exception, object? data = null, [CallerFilePath] string callerFilePath = "", [CallerMemberName] string callerMemberName = "") {
		AddProperties(data, _logger.WithProperty(CALLER, _CallerInfo(callerFilePath, callerMemberName))).Error(exception);
	...
	public void Info(string message, object? data = null, [CallerFilePath] string callerFilePath = "", [CallerMemberName] string callerMemberName = "") {
		...
		AddProperties(data, _logger.WithProperty(CALLER, _CallerInfo(callerFilePath, callerMemberName))).Info(message);
	...
	public LogBase Initialize(InitializeInfo initInfo, [CallerFilePath] string callerFilePath = "", [CallerMemberName] string callerMemberName = "") {
		ArgumentNullException.ThrowIfNull(initInfo);
		...
		_logger
			...
			.WithProperty(CALLER, _CallerInfo(callerFilePath, callerMemberName))
			.Info(INITIALIZED);
	...
	public void Warn(Exception? exception = null, string? message = null, object? data = null, [CallerFilePath] string callerFilePath = "", [CallerMemberName] string callerMemberName = "") {
		...
		AddProperties(data, _logger.WithProperty(CALLER, _CallerInfo(callerFilePath, callerMemberName))).Warn(exception, message);
	...
	protected virtual Logger AddProperties(object? data, Logger logger) {
		if (data == null)
			...
			return logger.WithProperty(DATA, data);
	}
